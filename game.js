...
function placeholderBackground(ctx, width, height) {
  ctx.save();
...
function placeholderGround(ctx, width, height, groundHeight) {
  ctx.save();
...
function placeholderBird(ctx, x, y, size) {
  ctx.save();
...
function frame(timestamp) {
  const seconds = lastTime ? Math.min((timestamp - lastTime) / 1000, 0.05) : 0;
  lastTime = timestamp;
  ...
  if (state === 'playing') {
    ...
    ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 48);
    ctx.fillText(String(score), CONFIG.canvasWidth / 2, 48);
    ctx.restore();
  }
